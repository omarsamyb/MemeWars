...
public class Game {
	...
	public void assemblePieces() {
		...
		Armored a1 = new Armored(player1, this, "Mortada_Red");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(a1);
		Armored a2 = new Armored(player2, this, "Mortada_Green");
		board[p2InitRow][ind2.remove(0)].setPiece(a2);

		Medic m1 = new Medic(player1, this, "M. A.ElGhany_R");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(m1);

		Medic m2 = new Medic(player2, this, "M. A.ElGhany_G");
		board[p2InitRow][ind2.remove(0)].setPiece(m2);

		Ranged r1 = new Ranged(player1, this, "A.AbdElAziz_Red");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(r1);

		Ranged r2 = new Ranged(player2, this, "A.AbdElAziz_Green");
		board[p2InitRow][ind2.remove(0)].setPiece(r2);

		Speedster sp1 = new Speedster(player1, this, "Big Shaq_Red");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(sp1);

		Speedster sp2 = new Speedster(player2, this, "Big Shaq_Green");
		board[p2InitRow][ind2.remove(0)].setPiece(sp2);

		Super su1 = new Super(player1, this, "H.El Wazeer_Red");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(su1);

		Super su2 = new Super(player2, this, "H.El Wazeer_Green");
		board[p2InitRow][ind2.remove(0)].setPiece(su2);

		Tech t1 = new Tech(player1, this, "Bayomi_Red");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(t1);

		Tech t2 = new Tech(player2, this, "Bayomi_Green");
		...
		for (int j = 0; j < boardWidth; j++) {
			board[p1InitRow][j].setPiece(new SideKickP1(this, "OFA-1" + j));
			board[p2InitRow + 1][j].setPiece(new SideKickP2(this, "OFA-2" + j));
		}
		for (int i = 0; i < board.length; i++)
