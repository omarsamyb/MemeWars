...
public abstract class Piece implements Movable {
	...
	public void attack(Piece target) {
		try {
			audioInputStream = AudioSystem.getAudioInputStream(Piece.class.getResourceAsStream("punch.wav"));
		...
		if (!(target instanceof Armored && ((Armored) target).isArmorUp())) {
			...
			try {
				audioInputStream = AudioSystem.getAudioInputStream(Piece.class.getResourceAsStream("mortadahit.wav"));
		        clip = AudioSystem.getClip();
		        clip.open(audioInputStream);
		        clip.start();
		    } catch(Exception ex) {
		        System.out.println("Error with playing soundt.");
		    }
	...
	public void MoveHelper(int newX, int newY, Direction d)
		...
		if ((occupantPiece != null && movingPiece.getOwner() != occupantPiece.owner)
			...
			throw new OccupiedCellException("This Cell Contains a Friendly Piece",this, d);
		}
